package com.hk_music_cop.demo.external.jandi.domain;
...
public class JandiRequestData {
	...
	public Params validParamCnt(String command, List<List<String>> parameters) {
		...
		if (!isValid) {
			throw new JandiUndefinedCommand("잘못된 명령어 입니다.");
		}
	...
	public FormatValidate validate(String requestData) {
		...
		for (int i = 0; i < requestData.length(); i++) {
			...
			if (c == '[') {
				if (flag) {
					throw new JandiUndefinedCommand("중첩된 대괄호는 허용되지 않습니다");
				}
				flag = true;
				openCount++;
				prePos = i;
				...
				if (!flag) {
					throw new JandiUndefinedCommand("닫는 대괄호가 열린 대괄호보다 먼저 나왔습니다");
				}
				flag = false;
				closeCount++;
		...
		if (flag) {
			throw new JandiUndefinedCommand("닫히지 않은 대괄호가 있습니다");
		}

		if (command == null) throw new JandiUndefinedCommand("명령어가 비어있습니다.");
